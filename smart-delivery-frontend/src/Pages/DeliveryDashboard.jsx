...
// // ייבוא הקומפוננטה של הצ'אט שבנינו
...
//   // פונקציה לריענון הנתונים - נקראת גם מהצ'אט!
//   const fetchData = async () => {
//     try {
//       setIsRefreshing(true);
//       const res = await axios.get(API_URL);
...
//       <div style={styles.scrollableArea}>
//         {filteredDeliveries.length === 0 ? (
//           <div style={styles.emptyState}><SearchX size={48} color="#334155" /><h3>לא נמצאו משלוחים</h3></div>
//         ) : (
//           viewType === 'grid' ? (
//             <div style={styles.grid}>
//               {filteredDeliveries.map((d) => (
...
//                     <div style={styles.badge(d.status)}>{d.status}</div>
//                   </div>
//                   <div style={styles.addr}><ArrowUpCircle size={16} color="#60a5fa" /><span>{d.pickupAddress}</span></div>
//                   <div style={styles.miniLine}></div>
//                   <div style={styles.addr}><ArrowDownCircle size={16} color="#f87171" /><span>{d.dropoffAddress}</span></div>
//                   <div style={styles.cardFooter}>
//                     <span style={styles.dateText}><Calendar size={12} style={{marginLeft:'5px'}} /> {new Date(d.createdAt).toLocaleDateString('he-IL')}</span>
//                     <div style={{display:'flex', gap:'10px'}}>
//                       <Edit2 size={16} onClick={() => openModal(d)} style={{cursor:'pointer', color: '#94a3b8'}} />
...
//                       <td style={styles.td}><span style={styles.badge(d.status)}>{d.status}</span></td>
//                       <td style={styles.td}>
//                         <div style={{display:'flex', gap:'15px'}}><Edit2 size={16} onClick={() => openModal(d)} style={{cursor:'pointer'}}/><Trash2 size={16} onClick={() => handleDelete(d.id)} style={{cursor:'pointer', color:'#ef4444'}}/></div>
//                       </td>
//                     </tr>
//                   ))}
//                 </tbody>
...
//       {/* Modal Section (שמרתי על הקוד המקורי שלך) */}
//       {showModal && (
//         <div style={styles.overlay}>
//           <div style={styles.modal}>
//             <form onSubmit={handleSave} style={styles.form}>
//                {/* ... תוכן המודל המקורי שלך ... */}
//                <div style={{display:'flex', justifyContent:'space-between', marginBottom:'15px'}}>
//                   <h2 style={{color:'#fff'}}>{currentDelivery.id ? 'עריכה' : 'חדש'}</h2>
//                   <X onClick={() => setShowModal(false)} style={{cursor:'pointer'}}/>
//                </div>
//                <label style={styles.modalLabel}>כתובת איסוף</label>
//                <input style={styles.input} value={currentDelivery.pickupAddress} onChange={e => setCurrentDelivery({...currentDelivery, pickupAddress: e.target.value})}/>
//                <label style={styles.modalLabel}>כתובת יעד</label>
//                <input style={styles.input} value={currentDelivery.dropoffAddress} onChange={e => setCurrentDelivery({...currentDelivery, dropoffAddress: e.target.value})}/>
...
//       {/* --- ה-AI CHAT BOX הצף --- */}
//       <div style={styles.aiChatPosition}>
//         <AIChatBox onDataChange={fetchData} />
//       </div>
...
//     padding: '40px', 
//     fontFamily: 'Heebo, sans-serif',
//     position: 'relative' // חשוב למיקום הצ'אט
//   },
//   aiChatPosition: {
//     position: 'fixed',
//     bottom: '30px',
//     left: '30px', // בפינה השמאלית כדי שלא יסתיר כפתורים
//     zIndex: 1000
//   },
//   // ... שאר הסטיילים המקוריים שלך ללא שינוי ...
//   scrollableArea: { marginTop: '20px', width: '100%' },
//   header: { display: 'flex', justifyContent: 'flex-end', marginBottom: '30px' },
//   refreshBtn: { background: 'none', border: 'none', cursor: 'pointer', padding: '10px' },
//   viewToggle: { display: 'flex', backgroundColor: '#1e293b', padding: '4px', borderRadius: '10px', marginLeft: '15px' },
//   viewBtn: { background: 'none', border: 'none', color: '#94a3b8', padding: '8px', cursor: 'pointer', borderRadius: '8px' },
...
//   }),
//   addr: { fontSize: '14px', color: '#e2e8f0', display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '5px' },
//   miniLine: { width: '2px', height: '12px', backgroundColor: '#334155', marginRight: '22px', margin: '4px 0' },
//   cardFooter: { display: 'flex', justifyContent: 'space-between', borderTop: '1px solid #334155', paddingTop: '15px', marginTop: '15px' },
//   dateText: { fontSize: '12px', color: '#64748b', display: 'flex', alignItems: 'center' },
//   tableWrapper: { backgroundColor: '#1e293b', borderRadius: '20px', border: '1px solid #334155', overflowX: 'auto' },
//   table: { width: '100%', borderCollapse: 'collapse', textAlign: 'right' },
...
//   td: { padding: '15px', fontSize: '14px', borderBottom: '1px solid #334155', color: '#e2e8f0' },
//   overlay: { position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.85)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000, backdropFilter: 'blur(6px)' },
//   modal: { backgroundColor: '#1e293b', padding: '30px', borderRadius: '24px', width: '480px', border: '1px solid #334155' },
//   modalLabel: { fontSize: '12px', color: '#94a3b8', marginTop: '15px', display: 'block' },
//   input: { padding: '12px', borderRadius: '12px', backgroundColor: '#0f172a', border: '1px solid #334155', color: '#fff', outline: 'none', marginTop: '5px', width: '100%' },
//   cancelBtn: { background: 'none', border: 'none', color: '#94a3b8', cursor: 'pointer', marginTop: '10px' },
//   loader: { height: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', color: '#fff' }
// };
